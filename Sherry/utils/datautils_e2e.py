...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            try:
                loacal_dataset = f"{args.dataset_root}/datasets/RedPajama-Data-1T-Sample/"
            ...
            dataset = load_from_disk(local_dataset)
        elif dataset_name in ['ultrafineweb-10b', "ultrafineweb-10b-qwen"]:
            local_dataset = f"{args.dataset_root}/datasets/Ultra-FineWeb-10B/"
            dataset = load_from_disk(local_dataset)
        elif dataset_name in ['ultrafineweb-100b']:
            local_dataset = f"{args.dataset_root}/datasets/Ultra-FineWeb-100B/"
            dataset = load_from_disk(local_dataset)
    ...
    def format_dataset(dataset, dataset_format):
        ...
        elif dataset_format == 'pt' or (dataset_format is None and args.dataset in ['c4', "redpajama-sample", 'redpajama-1b', 'redpajama-2b',
        'redpajama-10b',"ultrafineweb-10b","ultrafineweb-10b-qwen","ultrafineweb-100b",'slimpajama-14b', 'redpajama-10b-arxiv', 'redpajama-28b', 'redpajama-100b', 'redpajama-1t']):
            block_size = args.pt_context_len
            column_names = list(dataset["train"].features)
            text_column_name = "text" if "text" in column_names else column_names[0]

            def tokenize_function(examples):
                output = tokenizer(examples[text_column_name], padding="max_length", truncation=True, max_length=block_size)
                return output
    ...
    if args.dataset in ['c4', "wiki-10b", "redpajama-sample", 'redpajama-1b', 'redpajama-2b', 'redpajama-10b', "ultrafineweb-10b", "ultrafineweb-10b-qwen",
    "ultrafineweb-100b",'slimpajama-14b', 'redpajama-10b-arxiv', 'redpajama-28b','redpajama-100b', 'redpajama-1t',]:
        ...
        if os.path.exists(cache_dataloader):
            ...
            if args.begin_train_ratio != 0.0:
                ...
                dataset["train"] = dataset["train"].select(range(int(args.begin_train_ratio * len(dataset["train"])), len(dataset["train"])))
        else:
        ...
        train_dataset = dataset_cls(train_raw_data, tokenizer=tokenizer, conv_template=args.conv_temp,
                                    mask_user=args.mask_use)
        eval_dataset = dataset_cls(eval_raw_data, tokenizer=tokenizer, conv_template=args.conv_temp,
                                   mask_user=args.mask_use)
        ...
        if os.path.exists(cache_dataloader):
            ...
            train_dataset = concatenate_datasets(
                [deita_dataset, redpajama_dataset['train'].select(range(len(deita_dataset)))])
            dataset = {
                "train": train_dataset,
    ...
    if args.dataset in ['c4', 'wiki-10b','redpajama-1b', 'redpajama-2b', 'redpajama-10b', 'redpajama-10b-arxiv', 'ultrafineweb-10b' ,
     "ultrafineweb-10b-qwen", "ultrafineweb-100b",'slimpajama-14b', 'redpajama-28b','redpajama-100b', 'redpajama-1t', 'deita-6k', 'deita-10k', 'mix_deita_redpajama']:
